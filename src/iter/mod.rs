...
impl RawIter {
    #[must_use]
    #[inline]
    pub const fn from_list<U, A>(list: &DynList<U, A>) -> Self
    where
        U: ?Sized,
        A: Allocator,
    {
    ...
    #[must_use]
    #[inline]
    pub fn next<Metadata>(&mut self) -> Option<Node<Metadata>> {
        let Ends { front, back } = self.ends.as_mut()?;
        let node = unsafe { front.to_transparent() };
        ...
        if node.value_ptr() == back.value_ptr() {
            ...
            *front = unsafe { header.next.unwrap_unchecked() }.to_opaque();
        }
    ...
    #[must_use]
    #[inline]
    pub fn next_back<Metadata>(&mut self) -> Option<Node<Metadata>> {
        let Ends { front, back } = self.ends.as_mut()?;
        let node = unsafe { back.to_transparent() };
        ...
        if node.value_ptr() == front.value_ptr() {
            ...
            *back = unsafe { header.previous.unwrap_unchecked() }.to_opaque();
        }
